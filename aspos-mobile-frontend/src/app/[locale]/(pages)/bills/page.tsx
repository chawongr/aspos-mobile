  ...
  return (
    <div className="flex flex-col relative">
      ...
      <div className='flex justify-between items-center text-lg md:text-2xl font-medium py-4 md:py-6 border-b-[0.5px] border-borderGray'>
          <div>{t('order')} ({totalAmount.amount})</div>
          <div className='text-sm md:text-base text-darkGray bg-lightBg rounded-full px-2.5 py-1'>{t('table')} {totalAmount.table}</div>
      </div>

      <div className="flex-1 overflow-y-auto">
        {bills.map((bill, index) => (
          <div key={bill.id} className={`${index !== bills.length ? 'border-b-[0.5px] border-borderGray ' : ''}`} >
          <BillCard
            key={bill.id}
            imageUrl={Food2}
            title={bill.title}
